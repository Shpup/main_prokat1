            ...
            async refreshCalendar() {
                ...
                this.projects.forEach(project => {
                    window.calendar.addEvent({
                        ...
                        start: project.start_date,
                        end: project.end_date ? new Date(new Date(project.end_date).setDate(new Date(project.end_date).getDate() + 1)).toISOString().split('T')[0] : null,
                        url: "{{ route('projects.show', '') }}/" + project.id,
                        allDay: true,
                        color: project.status === 'active' ? 'rgba(34,197,94,0.71)' :
                            project.status === 'new' ? 'rgba(248,233,95,0.72)' :
                                project.status === 'completed' ? 'rgba(105,159,255,0.74)' :
                                    project.status === 'cancelled' ? 'rgba(255,87,87,0.76)' : '#9ca3af'
                    });
                });
            },
            sortBy(column) {
